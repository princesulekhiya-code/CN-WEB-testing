...
import { ArrowRight, ArrowLeft, Palette, Smartphone, Monitor, Layout, Settings, Gauge, Accessibility, ClipboardList, Code2, ShieldCheck, Rocket, CheckCircle2, Award, TrendingUp, Zap, HeartHandshake } from "lucide-react";
...
const services: ServiceOffering[] = [
  { icon: Palette, title: "Custom Theme Design", description: "Bespoke WordPress themes designed from scratch with unique layouts, brand-aligned aesthetics, and conversion-focused UX patterns.", features: ["Custom layouts", "Brand-aligned", "Conversion UX"] },
...
const processSteps = [
  { step: "01", title: "Design Research", description: "Analyze brand identity, target audience, competitors, and design trends for informed theme direction.", icon: ClipboardList },
...
const whyChoose = [
  { icon: Award, title: "Design Excellence", description: "Award-worthy theme designs that stand out — unique, on-brand, and built for real business outcomes." },
    ...
    <section className="min-h-screen bg-white text-black dark:bg-black dark:text-white">
      <div className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-6 pt-28 pb-20 relative">
          <Link href="/services/wordpress-development" className={`inline-flex items-center gap-2 text-sm font-medium text-black/50 hover:text-black dark:text-white/50 dark:hover:text-white transition-all duration-500 mb-8 ${heroReady ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-4"}`}><ArrowLeft size={16} /> Back to WordPress Development</Link>
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className={`text-4xl sm:text-5xl lg:text-[3.5rem] font-bold tracking-tight leading-[1.1] transition-all duration-700 delay-100 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Responsive Theme<br /><span className="text-[#4EB3E8]">Development</span></h1>
              <p className={`mt-6 text-lg font-medium text-black/50 dark:text-white/50 leading-relaxed max-w-lg transition-all duration-700 delay-200 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>UX designers and solution architects use design thinking to develop user-friendly, responsive WordPress themes that look stunning on every device.</p>
              ...
              >
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="absolute -top-4 -right-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">Custom Theme</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Responsive design</div>
                    </div>
                  </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className="absolute -bottom-4 -left-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">All Devices</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Mobile-first</div>
                    </div>
                  </div>
                </motion.div>
              </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Theme Development <span className="text-black dark:text-white">Services</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Custom, responsive, accessible WordPress themes — from mobile-first design to performance optimization.</p>
        </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden border-y border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-16">
        <motion.div className="text-center mb-20" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Our Theme Development <span className="text-black dark:text-white">Process</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto">From design research to performance-optimized, responsive WordPress themes.</p>
        </motion.div>
        <div className="relative">
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">WordPress theme APIs, responsive CSS, and modern testing tools for pixel-perfect themes.</p>
        </motion.div>

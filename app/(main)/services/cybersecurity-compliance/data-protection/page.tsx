...
const services: ServiceOffering[] = [
  {
    ...
    description: "Discover structured and unstructured data stores, label sensitivity, and document flows across SaaS, databases, and file shares.",
    features: ["Data inventory", "Lineage sketches", "Retention hooks"],
  },
  {
    icon: Lock,
    title: "Encryption at Rest & in Transit",
    description: "Apply modern cryptography defaults — TLS policies, database TDE, object storage SSE, and application-layer encryption where warranted.",
    features: ["TLS hardening", "KMS integration", "Field-level crypto"],
  },
  {
    icon: Shield,
    title: "DLP Policies",
    description: "Prevent exfiltration with endpoint, email, and cloud DLP tuned to reduce noise while protecting crown-jewel data classes.",
    features: ["Exfil patterns", "Just-in-time access", "Alert tuning"],
  },
  {
    icon: Eye,
    title: "Access Control & IAM",
    description: "Role design, separation of duties, privileged access workflows, and periodic access reviews aligned to least privilege.",
    features: ["RBAC/ABAC", "PAM patterns", "Access reviews"],
  },
  {
    icon: KeyRound,
    title: "Key Management",
    description: "Centralized key lifecycle, rotation, and segregation of duties between data owners and platform operators.",
    features: ["HSM/KMS", "Rotation policy", "Break-glass"],
  },
  {
    icon: Sparkles,
    title: "Privacy-by-Design",
    description: "Embed minimization, purpose limitation, and transparency into product flows — from analytics to customer support tooling.",
    features: ["Consent UX", "PII minimization", "DSAR readiness"],
  },
];

const processSteps = [
  { step: "01", title: "Data Discovery", description: "Automated scanning plus stakeholder interviews to locate regulated and confidential data.", icon: Map },
  { step: "02", title: "Classification Scheme", description: "Define labels, handling rules, and approved locations per data class.", icon: Tags },
  { step: "03", title: "Control Design", description: "Encryption, DLP, IAM, and logging controls mapped to each sensitivity tier.", icon: Layers },
  { step: "04", title: "Implementation", description: "Roll out policies with phased enforcement — monitor mode first, then progressive blocking.", icon: Settings },
  { step: "05", title: "Validation", description: "Test cases for exfil paths, key rotation, and backup/restore integrity.", icon: FileKey },
  { step: "06", title: "Operational Playbooks", description: "Runbooks for incidents involving keys, DLP alerts, and access anomalies.", icon: ClipboardList },
];

const techCategories = [
  { label: "Encryption & Keys", items: [
...
const whyChoose = [
  { icon: Users, title: "Cross-Functional Team", description: "Security engineers paired with privacy-minded architects — fewer handoffs, faster decisions." },
  { icon: Clock, title: "Phased Rollouts", description: "Reduce business disruption with staged enforcement and measurable false-positive budgets." },
  { icon: Award, title: "Framework Alignment", description: "Mappings to GDPR, HIPAA, and PCI expectations baked into deliverables." },
  { icon: HeartHandshake, title: "Engineer-Friendly", description: "SDK patterns, IaC snippets, and CI checks your developers can adopt without guesswork." },
];
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className={`text-4xl sm:text-5xl lg:text-[3.5rem] font-bold tracking-tight leading-[1.1] transition-all duration-700 delay-100 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
                Data Protection
                <br />
                <span className="text-[#4EB3E8]">& Encryption</span>
              </h1>
              ...
              >
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="absolute -top-4 -right-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">Encryption</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">End-to-end</div>
                    </div>
                  </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className="absolute -bottom-4 -left-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">Zero Breach</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Data secured</div>
                    </div>
                  </div>
                </motion.div>
              </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Data Protection & <span className="text-black dark:text-white">Encryption</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Protect sensitive information with layered controls — from discovery and classification through cryptographic enforcement and operational guardrails.</p>
        </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden border-y border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-16">
        <motion.div className="text-center mb-20" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Our Data Protection <span className="text-black dark:text-white">Process</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto">A pragmatic path from visibility to enforcement — with privacy principles embedded in how your product and platforms evolve.</p>
        </motion.div>
        <div className="relative">
...
function TechStackSection() {
  ...
  return (
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Data Protection <span className="text-black dark:text-white">Technologies</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Platforms commonly integrated for discovery, encryption, DLP, and identity-backed access.</p>
        </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20 text-center">
        <h2 className={`text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
          Ready to Protect Sensitive
          <br />
          <span className="text-[#4EB3E8]">Data at Scale?</span>
        </h2>

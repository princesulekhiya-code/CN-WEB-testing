...
const services: ServiceOffering[] = [
  { icon: Code2, title: "Code Refactoring", description: "Reduce complexity, remove dead paths, and align modules with domain boundaries for safer change.", features: ["Modularization", "Test seams", "Tech debt backlog"] },
  { icon: Gauge, title: "Performance Optimization", description: "Profile hot paths, tune queries and caches, and right-size infrastructure for latency and cost.", features: ["Profiling", "Caching", "Query tuning"] },
  { icon: Sparkles, title: "Feature Enhancement", description: "Ship new capabilities on a stable core with flags, incremental rollout, and telemetry-backed validation.", features: ["Feature flags", "Telemetry", "Rollouts"] },
  { icon: Layers, title: "Tech Stack Upgrade", description: "Framework and runtime upgrades with compatibility shims, codemods, and staged adoption.", features: ["Codemods", "Dual-run", "Upgrade playbooks"] },
  { icon: Database, title: "Database Re-engineering", description: "Schema evolution, indexing strategy, and migration paths that protect data and uptime.", features: ["Schema migrations", "Indexing", "Replication"] },
  { icon: ShieldCheck, title: "Security Hardening", description: "Threat modeling, dependency hygiene, secrets management, and secure SDLC integration.", features: ["SAST/DAST", "Secrets", "Patch cadence"] },
];

const processSteps = [
  { step: "01", title: "Product Assessment", description: "Understand user pain, code health, and operational risk to prioritize re-engineering bets.", icon: ClipboardList },
  { step: "02", title: "Re-engineering Strategy", description: "Define target architecture, milestones, and interfaces that minimize disruption.", icon: Map },
  { step: "03", title: "Code Refactoring", description: "Incremental refactors behind stable APIs with tests and observability to catch regressions early.", icon: Code2 },
  { step: "04", title: "Feature Development", description: "Build and ship enhancements on the modernized foundation with CI/CD guardrails.", icon: Hammer },
  { step: "05", title: "Testing & QA", description: "Automated unit, integration, and performance testing plus exploratory QA for critical journeys.", icon: TestTube2 },
  { step: "06", title: "Release & Monitor", description: "Progressive delivery with dashboards, alerts, and rollback-ready releases.", icon: Activity },
];
...
const whyChoose = [
  { icon: Shield, title: "Minimal Disruption", description: "Phased delivery, parallel runs, and feature flags so users keep working while systems improve." },
  { icon: TrendingUp, title: "Performance Gains", description: "Measurable improvements to latency, throughput, and cost after each milestone." },
  { icon: Layers, title: "Modern Codebase", description: "Readable structure, automated tests, and conventions your team can extend with confidence." },
  { icon: Zap, title: "Faster Releases", description: "CI/CD, smaller batches, and observability that shrink cycle time from commit to production." },
];
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-6 pt-28 pb-20 relative">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className={`text-4xl sm:text-5xl lg:text-[3.5rem] font-bold tracking-tight leading-[1.1] transition-all duration-700 delay-100 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Product<br /><span className="text-[#4EB3E8]">Re-engineering</span></h1>
              <p className={`mt-6 text-lg font-medium text-black/50 dark:text-white/50 leading-relaxed max-w-lg transition-all duration-700 delay-200 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Enhance legacy products with new features, performance optimization, and faster speed-to-market through comprehensive re-engineering.</p>
              ...
              >
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="absolute -top-4 -right-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#4EB3E8]/10">
                      <Zap className="w-4 h-4 text-[#4EB3E8]" />
                    </div>
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">Re-Engineer</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Product rebuild</div>
                    </div>
                  </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className="absolute -bottom-4 -left-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">2x Faster</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Performance gain</div>
                    </div>
                  </div>
                </motion.div>
              </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Product Re-engineering <span className="text-black dark:text-white">Services</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Modernize legacy products with disciplined refactoring, performance work, and secure delivery practices.</p>
        </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden border-y border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-16">
        <motion.div className="text-center mb-20" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Our Product Re-engineering <span className="text-black dark:text-white">Process</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto">Assessment through release — structured delivery that balances velocity with stability.</p>
        </motion.div>
        <div className="relative">
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Languages, frameworks, data stores, testing, and DevOps aligned to maintainable product delivery.</p>
        </motion.div>
  ...
  return (
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20 text-center">
        <h2 className={`text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Ready to Re-engineer Your<br /><span className="text-[#4EB3E8]">Product?</span></h2>
        <p className={`mt-5 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-xl mx-auto leading-relaxed transition-all duration-700 delay-150 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Ship faster on a cleaner codebase with performance and security baked into every release.</p>

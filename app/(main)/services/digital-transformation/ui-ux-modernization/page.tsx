...
const services: ServiceOffering[] = [
  { icon: Palette, title: "Design System Creation", description: "Tokens, components, and documentation that keep product teams aligned and shipping faster.", features: ["Tokens & themes", "Component lib", "Figma kits"] },
  { icon: FileSearch, title: "UX Audit & Redesign", description: "Heuristic reviews, journey maps, and prioritized fixes that lift conversion and satisfaction.", features: ["Heuristics", "Journeys", "Prioritized backlog"] },
  { icon: Layout, title: "Responsive Modernization", description: "Fluid layouts, breakpoints, and performance budgets that feel great on every device.", features: ["Breakpoints", "Touch targets", "Perf budgets"] },
  { icon: Accessibility, title: "Accessibility Upgrade", description: "WCAG-aligned patterns, keyboard flows, and assistive-tech testing baked into delivery.", features: ["WCAG 2.1", "Keyboard", "Screen readers"] },
  { icon: Gauge, title: "Performance UX Optimization", description: "Perceived speed, skeleton states, and interaction design that reduce abandonment.", features: ["Core Web Vitals", "Skeletons", "Lazy media"] },
  { icon: MousePointerClick, title: "Interactive Prototype", description: "Clickable prototypes and usability loops before engineering commits to scope.", features: ["Hi-fi prototypes", "User tests", "Iterations"] },
];

const processSteps = [
  { step: "01", title: "UX Audit", description: "Baseline the experience with analytics, session replay, and qualitative review of friction points.", icon: FileSearch },
  { step: "02", title: "User Research", description: "Interviews, surveys, and usability sessions to validate problems and success metrics.", icon: Users },
  { step: "03", title: "Design Strategy", description: "Information architecture, flows, and visual direction aligned to brand and outcomes.", icon: Compass },
  { step: "04", title: "Prototype & Test", description: "Interactive prototypes with rapid test cycles to de-risk the build.", icon: PenTool },
  { step: "05", title: "Implementation", description: "Design-to-dev handoff with specs, tokens, and QA against acceptance criteria.", icon: Hammer },
  { step: "06", title: "Measure & Iterate", description: "Instrumentation, A/B tests, and continuous improvement against KPIs.", icon: BarChart3 },
];
...
const whyChoose = [
  { icon: Palette, title: "Design-Led Thinking", description: "Problems framed from user and business outcomes before pixels hit the screen." },
  { icon: LineChart, title: "Data-Driven Decisions", description: "Quant and qual signals guide prioritization so every sprint moves metrics." },
  { icon: Eye, title: "Accessibility First", description: "Inclusive patterns by default — not as an afterthought before launch." },
  { icon: BarChart3, title: "Measurable Impact", description: "KPIs, experimentation, and dashboards tied to UX investments." },
];

const heroCards = [
  { icon: Palette, label: "Design" },
  { icon: Layout, label: "Responsive" },
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-6 pt-28 pb-20 relative">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className={`text-4xl sm:text-5xl lg:text-[3.5rem] font-bold tracking-tight leading-[1.1] transition-all duration-700 delay-100 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>UI/UX<br /><span className="text-[#4EB3E8]">Modernization</span></h1>
              <p className={`mt-6 text-lg font-medium text-black/50 dark:text-white/50 leading-relaxed max-w-lg transition-all duration-700 delay-200 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Adopt the latest design trends and experiences to create engaging products that boost user engagement and business value.</p>
              ...
              >
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="absolute -top-4 -right-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#4EB3E8]/10">
                      <Palette className="w-4 h-4 text-[#4EB3E8]" />
                    </div>
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">UX Refresh</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Modern design</div>
                    </div>
                  </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className="absolute -bottom-4 -left-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">60% Better</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">User engagement</div>
                    </div>
                  </div>
                </motion.div>
              </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">UI/UX Modernization <span className="text-black dark:text-white">Services</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Design systems, research, accessibility, and performance UX — unified for products users love.</p>
        </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden border-y border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-16">
        <motion.div className="text-center mb-20" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Our UI/UX Modernization <span className="text-black dark:text-white">Process</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto">Research, strategy, prototyping, build, and continuous measurement — in one loop.</p>
        </motion.div>
        <div className="relative">
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Design tools, frontend stacks, testing, accessibility, and analytics for modern UX delivery.</p>
        </motion.div>
  ...
  return (
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20 text-center">
        <h2 className={`text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Ready to Elevate Your<br /><span className="text-[#4EB3E8]">UI/UX Experience?</span></h2>
        <p className={`mt-5 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-xl mx-auto leading-relaxed transition-all duration-700 delay-150 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Partner with us to modernize design systems, accessibility, and product experiences that convert.</p>

        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-5">
          {industries.map((ind, i) => {
            ...
            return (
              <div
                key={ind.name}
                className={`group flex flex-col items-center p-6 rounded-2xl border border-black/[0.04] dark:border-white/[0.04] hover:border-[#006ea3]/20 hover:bg-[#006ea3]/[0.02] transition-all duration-500 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}
                style={{ transitionDelay: `${200 + i * 60}ms` }}
              >
                <div className="mb-3 flex h-11 w-11 items-center justify-center rounded-xl bg-[#006ea3]/10 border border-[#006ea3]/20 group-hover:scale-110 transition-transform duration-300">
                  <Icon className="w-5 h-5 text-[#006ea3]" strokeWidth={1.5} />
                </div>
                <span className="text-sm font-semibold text-black/60 dark:text-white/60 group-hover:text-[#006ea3] transition-colors duration-300">{ind.name}</span>
              </div>
            );
          })}
        </div>

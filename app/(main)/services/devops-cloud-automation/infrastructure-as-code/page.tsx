...
const services: ServiceOffering[] = [
  { icon: Blocks, title: "Terraform Module Development", description: "Composable modules with clear interfaces, versioning, and documentation so teams reuse infrastructure safely.", features: ["Module registry", "Contract tests", "Upgrade paths"] },
  { icon: Cloud, title: "Cloud Infrastructure Provisioning", description: "Repeatable environments across accounts and regions with guardrails for networking, identity, and data.", features: ["Landing zones", "Policy as code", "Cost tags"] },
  { icon: SlidersHorizontal, title: "Configuration Management", description: "Drift-resistant configuration for VMs, Kubernetes add-ons, and hybrid estates where imperative still matters.", features: ["Idempotent runs", "Inventory", "Secrets integration"] },
  { icon: ScanSearch, title: "State Management & Drift Detection", description: "Remote state, locking, and proactive drift detection so changes never surprise production on a Friday.", features: ["State hygiene", "Drift alerts", "Run auditing"] },
  { icon: Globe, title: "Multi-Cloud IaC Strategy", description: "Abstractions and workflows that reduce lock-in while staying pragmatic about cloud-native primitives.", features: ["Reference arch", "Shared modules", "Cloud adapters"] },
  { icon: ShieldCheck, title: "IaC Testing & Validation", description: "Static analysis, policy checks, and integration tests that catch issues before `apply` touches shared environments.", features: ["Policy gates", "Plan reviews", "Test harnesses"] },
];

const processSteps = [
  { step: "01", title: "Inventory & Goals", description: "Document existing resources, blast radius, compliance needs, and the outcomes you want from codification.", icon: Target },
  { step: "02", title: "IaC Blueprint", description: "Pick toolchain, module boundaries, state strategy, and CI patterns for plan/apply with approvals.", icon: FileCode },
  { step: "03", title: "Build & Migrate", description: "Implement modules, import legacy resources where needed, and validate parity with careful cutovers.", icon: Plug },
  { step: "04", title: "Testing & Policy", description: "Wire scanners, OPA/Sentinel policies, and automated plan checks into every pull request.", icon: RefreshCw },
  { step: "05", title: "Operational Model", description: "Define ownership, access, break-glass, and observability for infrastructure changes at scale.", icon: Scale },
  { step: "06", title: "Enablement", description: "Train teams, publish patterns, and iterate on developer experience so IaC stays the default path.", icon: Rocket },
];

const techCategories = [
  { label: "IaC Tools", items: [
...
const whyChoose = [
  ...
  { icon: HeartHandshake, title: "Knowledge Transfer", description: "Runbooks, templates, and team enablement so your engineers own and extend infrastructure code long term." },
];
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-6 pt-28 pb-20 relative">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className={`text-4xl sm:text-5xl lg:text-[3.5rem] font-bold tracking-tight leading-[1.1] transition-all duration-700 delay-100 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Infrastructure<br /><span className="text-[#4EB3E8]">as Code</span></h1>
              <p className={`mt-6 text-lg font-medium text-black/50 dark:text-white/50 leading-relaxed max-w-lg transition-all duration-700 delay-200 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Treat infrastructure like application code — reviewed, tested, and versioned — so every environment is reproducible and auditable.</p>
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="absolute -top-4 -right-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    <div className="flex h-9 w-9 items-center justify-center rounded-lg bg-[#4EB3E8]/10">
                      <Globe className="w-4 h-4 text-[#4EB3E8]" />
                    </div>
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">Terraform</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">IaC automation</div>
                    </div>
                  </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className="absolute -bottom-4 -left-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Infra provisioning</div>
                    </div>
                  </div>
                </motion.div>
              </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Six focused capabilities covering modules, provisioning, configuration, state, multi-cloud strategy, and validation.</p>
        </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden border-y border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-16">
        <motion.div className="text-center mb-20" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto">Six phases on an alternating vertical timeline.</p>
        </motion.div>
        <div className="relative">
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Representative tools by category.</p>
        </motion.div>
  ...
  return (
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20 text-center">
        <h2 className={`text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Ready to Codify Your<br /><span className="text-[#4EB3E8]">Infrastructure?</span></h2>
        <p className={`mt-5 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-xl mx-auto leading-relaxed transition-all duration-700 delay-150 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Partner with us to make infrastructure changes reviewable, testable, and boringly reliable across teams and clouds.</p>

...
const services: ServiceOffering[] = [
  { icon: Box, title: "Docker Containerization", description: "Production-ready images with minimal attack surface, reproducible builds, and clear supply-chain metadata.", features: ["Multi-stage builds", "SBOM hooks", "Base image policy"] },
  { icon: Boxes, title: "Kubernetes Cluster Management", description: "Cluster lifecycle, upgrades, node pools, and reliability patterns tuned for your workloads and SLOs.", features: ["HA control plane", "Upgrades", "Capacity planning"] },
  { icon: Package, title: "Helm Chart Development", description: "Versioned charts, values layering, and release automation that keep environments consistent and auditable.", features: ["Chart testing", "GitOps hooks", "Rollback paths"] },
  { icon: Network, title: "Service Mesh Implementation", description: "mTLS, traffic policies, and observability across services without scattering cross-cutting logic in app code.", features: ["Traffic shaping", "Retries/timeouts", "Golden signals"] },
  { icon: Container, title: "Container Registry Management", description: "Signing, retention, vulnerability workflows, and promotion rules from dev artifacts to production.", features: ["Image signing", "CVE gates", "Promotion flows"] },
  { icon: Globe2, title: "Multi-Cluster & Federation", description: "Patterns for active-active, disaster recovery, and fleet operations when one cluster is not enough.", features: ["Fleet policy", "DR drills", "Ingress strategy"] },
];

const processSteps = [
  { step: "01", title: "Workload Discovery", description: "Inventory services, dependencies, data gravity, and non-negotiables like compliance and latency.", icon: Target },
  { step: "02", title: "Container Baseline", description: "Define image standards, CI build steps, and registry strategy aligned to your security posture.", icon: Box },
  { step: "03", title: "Cluster Design", description: "Networking, identity, autoscaling, and add-ons chosen for operability at day two and beyond.", icon: Server },
  { step: "04", title: "Implementation", description: "Roll out workloads, ingress, secrets, and GitOps workflows with safe promotion paths.", icon: Plug },
  { step: "05", title: "Mesh & Hardening", description: "Optional mesh rollout, policy guardrails, and chaos-ready health checks before peak traffic.", icon: Layers },
  { step: "06", title: "Operational Handover", description: "Dashboards, runbooks, and training so your platform team can run the stack confidently.", icon: Rocket },
];
...
const techCategories = [
  ...
  { label: "Package Management", items: [
...
const whyChoose = [
  ...
  { icon: HeartHandshake, title: "Knowledge Transfer", description: "Runbooks, templates, and team enablement so your engineers own and extend the platform long term." },
];

const heroCards = [
  { icon: Box, label: "Containers" },
  { icon: Layers, label: "Orchestration" },
  { icon: Server, label: "Clusters" },
  { icon: Network, label: "Service Mesh" },
];
      ...
      <div className="relative overflow-hidden">
        ...
        <div className="mx-auto max-w-7xl px-6 pt-28 pb-20 relative">
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className={`text-4xl sm:text-5xl lg:text-[3.5rem] font-bold tracking-tight leading-[1.1] transition-all duration-700 delay-100 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Containerization<br /><span className="text-[#4EB3E8]">& Orchestration</span></h1>
              <p className={`mt-6 text-lg font-medium text-black/50 dark:text-white/50 leading-relaxed max-w-lg transition-all duration-700 delay-200 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Package applications consistently, run them at scale on Kubernetes, and add mesh-level controls when traffic complexity demands it.</p>
              ...
              >
                <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl shadow-black/10 dark:shadow-black/40 border border-black/[0.06] dark:border-white/[0.06]">
                  <Image
                    ...
                    alt="Docker & Kubernetes Containers"
                    fill
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="absolute -top-4 -right-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">Docker & K8s</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Container experts</div>
                    </div>
                  </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className="absolute -bottom-4 -left-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">Auto-Scale</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Elastic infra</div>
                    </div>
                  </div>
                </motion.div>
              </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          ...
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Six focused capabilities covering images, Kubernetes operations, packaging, mesh, registries, and multi-cluster patterns.</p>
        </motion.div>
  ...
  return (
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20 text-center">
        <h2 className={`text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Ready to Containerize Your<br /><span className="text-[#4EB3E8]">Applications?</span></h2>
        <p className={`mt-5 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-xl mx-auto leading-relaxed transition-all duration-700 delay-150 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Let us help you standardize packaging, streamline Kubernetes operations, and scale confidently across environments.</p>

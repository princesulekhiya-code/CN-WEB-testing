...
import {
  ArrowRight, ArrowLeft, Server, Cpu, Network, Shield,
  CheckCircle2, ClipboardList, Settings, Rocket, Layers, Zap,
...
const services: ServiceOffering[] = [
  { icon: Server, title: "Cloud Data Centre Design", description: "Virtual data centre architecture with compute, storage, and networking optimized for your workload profiles and growth trajectory.", features: ["VPC design", "Compute sizing", "Storage tiers"] },
  { icon: Network, title: "Network Architecture", description: "VPC design, subnet planning, load balancing, and hybrid connectivity patterns that balance security with performance.", features: ["VPC/subnets", "Load balancing", "Hybrid connectivity"] },
  { icon: Layers, title: "Infrastructure as Code", description: "Terraform and CloudFormation modules that make infrastructure repeatable, versioned, drift-detected, and auditable.", features: ["Terraform modules", "Drift detection", "Version control"] },
  { icon: Shield, title: "Security Infrastructure", description: "Firewalls, WAF, encryption at rest and in transit, IAM policies, and security group architecture for defense in depth.", features: ["WAF & firewalls", "Encryption", "IAM hardening"] },
  { icon: Cpu, title: "Hybrid & Multi-Cloud Infrastructure", description: "Workload portability, shared services, and egress optimization across multiple cloud providers and on-prem environments.", features: ["Multi-cloud", "On-prem bridge", "Egress optimization"] },
  { icon: Zap, title: "Edge & CDN Infrastructure", description: "Global edge locations, CDN configuration, and edge compute for latency-sensitive applications and content delivery.", features: ["Global CDN", "Edge compute", "Latency optimization"] },
];

const processSteps = [
  { step: "01", title: "Requirements Gathering", description: "Workload profiles, compliance needs, performance targets, and budget constraints to shape infrastructure design.", icon: ClipboardList },
  { step: "02", title: "Architecture Design", description: "Multi-AZ, multi-region architecture with network topology, security boundaries, and HA patterns documented.", icon: Layers },
  { step: "03", title: "IaC Development", description: "Terraform modules, environment templates, and CI/CD pipelines for automated infrastructure provisioning.", icon: Settings },
  { step: "04", title: "Security Hardening", description: "IAM policies, encryption, network ACLs, WAF rules, and compliance baseline configuration.", icon: Shield },
  { step: "05", title: "Provisioning & Testing", description: "Infrastructure deployment, connectivity testing, failover drills, and performance validation.", icon: Rocket },
  { step: "06", title: "Operations Handoff", description: "Monitoring setup, runbook documentation, team training, and SLA-backed operational support.", icon: Server },
];
...
const whyChoose = [
  { icon: Users, title: "Certified Infrastructure Architects", description: "AWS, Azure, and GCP certified architects with deep networking and security expertise." },
  { icon: Clock, title: "Rapid Provisioning", description: "IaC-first approach means new environments in minutes, not weeks — repeatable and auditable." },
  { icon: Award, title: "Enterprise-Grade Design", description: "Multi-AZ, multi-region architectures designed for 99.99% availability and disaster resilience." },
  { icon: HeartHandshake, title: "Ongoing Infrastructure Support", description: "Managed services, patching, scaling, and cost optimization under SLA commitments." },
];
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className={`text-4xl sm:text-5xl lg:text-[3.5rem] font-bold tracking-tight leading-[1.1] transition-all duration-700 delay-100 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
                Cloud Infrastructure<br /><span className="text-[#4EB3E8]">& Implementation</span>
              </h1>
              ...
              >
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="absolute -top-4 -right-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">Multi-Cloud</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">AWS, Azure, GCP</div>
                    </div>
                  </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className="absolute -bottom-4 -left-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">99.99% SLA</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Enterprise uptime</div>
                    </div>
                  </div>
                </motion.div>
              </motion.div>
...
function HeroStats({ heroReady }: { heroReady: boolean }) {
  ...
  const items = [{ icon: Server, label: "Data Centre" }, { icon: Network, label: "Networking" }, { icon: Layers, label: "IaC" }, { icon: Shield, label: "Security" }];
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Infrastructure <span className="text-black dark:text-white">Services</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Complete cloud infrastructure — from data centre design to edge computing — built for security, scale, and operational efficiency.</p>
        </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden border-y border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-16">
        <motion.div className="text-center mb-20" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Our Infrastructure <span className="text-black dark:text-white">Process</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto">From requirements through operations — a structured approach to building infrastructure you can trust.</p>
        </motion.div>
        <div className="relative">
...
function TechStackSection() {
  ...
  return (
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Infrastructure <span className="text-black dark:text-white">Technologies</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Enterprise cloud compute, networking, storage, and automation platforms for resilient infrastructure.</p>
        </motion.div>
  ...
  return (
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20 text-center">
        <h2 className={`text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Ready to Build Your<br /><span className="text-[#4EB3E8]">Cloud Infrastructure?</span></h2>
        <p className={`mt-5 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-xl mx-auto leading-relaxed transition-all duration-700 delay-150 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Share your workload requirements — we&apos;ll design a cloud infrastructure that scales with your business.</p>

...
import {
  ArrowRight, ArrowLeft, Cloud, Server, Database, Gauge,
  CheckCircle2, ClipboardList, Settings, Rocket, Monitor, Zap,
  Award, HeartHandshake, Users, ShieldCheck,
} from "lucide-react";
...
const services: ServiceOffering[] = [
  { icon: Cloud, title: "Cloud-Native App Development", description: "Build applications designed for the cloud from day one — microservices, containers, and serverless patterns for maximum scalability.", features: ["Microservices", "Containers", "Serverless"] },
  { icon: Server, title: "Application Migration to Cloud", description: "Migrate legacy applications to IaaS or PaaS platforms with minimal downtime, data integrity checks, and rollback plans.", features: ["Lift & shift", "Re-platforming", "Zero-downtime"] },
  { icon: Monitor, title: "Application Monitoring & APM", description: "Real-time performance monitoring, alerting, and distributed tracing to keep cloud applications healthy and responsive.", features: ["APM tools", "Distributed tracing", "Alert pipelines"] },
  { icon: Database, title: "Cloud Database Management", description: "Managed database services, replication, backup strategies, and performance tuning across SQL and NoSQL platforms.", features: ["Managed DBs", "Replication", "Auto-scaling"] },
  { icon: Gauge, title: "Performance Optimization", description: "Auto-scaling, caching, CDN configuration, and code-level optimizations to ensure cloud apps perform under load.", features: ["Auto-scaling", "Caching", "CDN tuning"] },
  { icon: Settings, title: "Application Modernization", description: "Decompose monoliths into microservices, containerize workloads, and adopt CI/CD for faster, safer releases.", features: ["Decomposition", "Containerization", "CI/CD"] },
];

const processSteps = [
  { step: "01", title: "Application Assessment", description: "Inventory applications, map dependencies, and classify workloads by migration strategy.", icon: ClipboardList },
  { step: "02", title: "Architecture Planning", description: "Target-state architecture, service boundaries, and integration patterns for cloud-native delivery.", icon: Settings },
  { step: "03", title: "Development & Migration", description: "Iterative build or migration with automated testing and rehearsed cutovers.", icon: Rocket },
  { step: "04", title: "Integration & Testing", description: "API integration, load testing, and security validation before production rollout.", icon: Zap },
  { step: "05", title: "Deployment & Monitoring", description: "Blue-green or canary deployments with full observability from day one.", icon: Monitor },
  { step: "06", title: "Optimization & Support", description: "Continuous performance tuning, cost optimization, and SLA-backed operational support.", icon: Gauge },
];
...
const whyChoose = [
  { icon: Users, title: "Full-Stack Cloud Experts", description: "Engineers proficient across front-end, back-end, and cloud infrastructure for truly integrated solutions." },
  { icon: ShieldCheck, title: "Enterprise-Grade Security", description: "Security baked into every layer — encryption, access controls, and compliance from the start." },
  { icon: Award, title: "Proven Migration Track Record", description: "100+ successful application migrations with zero data loss and minimal business disruption." },
  { icon: HeartHandshake, title: "Ongoing Managed Services", description: "Post-launch monitoring, patching, scaling, and feature delivery under SLA commitments." },
];
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              <h1 className={`text-4xl sm:text-5xl lg:text-[3.5rem] font-bold tracking-tight leading-[1.1] transition-all duration-700 delay-100 ${heroReady ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>
                Cloud Application<br /><span className="text-[#4EB3E8]">Services</span>
              </h1>
              ...
              >
                <div className="relative w-full h-full rounded-2xl overflow-hidden shadow-2xl shadow-black/10 dark:shadow-black/40 border border-black/[0.06] dark:border-white/[0.06]">
                  <Image
                    ...
                    alt="Cloud Application Services"
                    fill
                ...
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.8 }} className="absolute -top-4 -right-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">Cloud Apps</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">SaaS solutions</div>
                    </div>
                  </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 1.0 }} className="absolute -bottom-4 -left-4 px-4 py-3 rounded-xl bg-white/95 dark:bg-[#111]/95 backdrop-blur-xl border border-black/[0.06] dark:border-white/[0.08] shadow-xl shadow-black/10 dark:shadow-black/30">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-sm font-bold text-black dark:text-white">99.9% Uptime</div>
                      <div className="text-[10px] font-medium text-black/40 dark:text-white/40">Enterprise SLA</div>
                    </div>
                  </div>
                </motion.div>
              </motion.div>
...
function HeroStats({ heroReady }: { heroReady: boolean }) {
  ...
  const items = [{ icon: Cloud, label: "Cloud-Native" }, { icon: Server, label: "Migration" }, { icon: Monitor, label: "Monitoring" }, { icon: Gauge, label: "Performance" }];
    ...
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Application <span className="text-black dark:text-white">Services</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Full-lifecycle cloud application services — from greenfield development to legacy modernization and managed operations.</p>
        </motion.div>
    ...
    <div ref={ref} className="relative overflow-hidden border-y border-black/[0.06] dark:border-white/[0.06]">
      ...
      <div className="relative mx-auto max-w-5xl px-6 py-16">
        <motion.div className="text-center mb-20" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Our Application <span className="text-black dark:text-white">Process</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto">A proven delivery model — assess, plan, build, deploy, optimize — for cloud applications that perform.</p>
        </motion.div>
        <div className="relative">
...
function TechStackSection() {
  ...
  return (
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-16">
        <motion.div className="text-center mb-14" initial={{ opacity: 0, y: 24 }} animate={visible ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight text-[#999] dark:text-[#8b8b8b]">Application Platforms & <span className="text-black dark:text-white">Tools</span></h2>
          <p className="mt-4 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-2xl mx-auto leading-relaxed">Cloud-native platforms, container orchestration, and observability tools for resilient applications.</p>
        </motion.div>
  ...
  return (
    <div ref={ref} className="relative overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-6 py-20 text-center">
        <h2 className={`text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight transition-all duration-700 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-6"}`}>Ready to Build Cloud-Native<br /><span className="text-[#4EB3E8]">Applications?</span></h2>
        <p className={`mt-5 text-base md:text-lg font-medium text-black/50 dark:text-white/50 max-w-xl mx-auto leading-relaxed transition-all duration-700 delay-150 ${visible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-4"}`}>Let&apos;s discuss your application landscape and chart the fastest path to cloud-native delivery.</p>
